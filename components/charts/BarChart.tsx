...
export function BarChart({ data, height = 120, color = C.cyan }: BarChartProps) {
    ...
    return (
        <View style={{ width: '100%' }}>
                ...
                {data.map((d, i) => {
                    ...
                    return (
                        <View key={d.label + i} style={{ flex: 1, alignItems: 'center' }}>
                            <Text style={{ color: C.text, fontSize: 10, fontWeight: '800', marginBottom: 4 }}>
                                {d.value > 0 ? d.value : ''}
                            </Text>
                            <Svg width="100%" height={barHeight} viewBox={`0 0 20 ${Math.max(barHeight, 3)}`} preserveAspectRatio="none">
                                <Rect
                                    x={2} y={0} width={16} height={Math.max(barHeight, 3)} rx={4}
            ...
            <View style={{ flexDirection: 'row', gap: barGap, marginTop: 8 }}>
                {data.map((d, i) => (
                    <Text key={d.label + i} style={{ flex: 1, textAlign: 'center', color: C.sub, fontSize: 9, fontWeight: '600' }}>
                        {d.label}
                    </Text>
                ))}
            </View>
