 ...
 * 2026-07-03 — NEW
 *
 * WHERE THIS FITS: `app/(tabs)/configure.tsx` already has a full flat
 * Engine + Rules CRUD screen (LOCATION_PRESETS, WORK_TYPE_OPTIONS,
 * EXPERIENCE_LEVELS are already defined there and reused here so both
 * screens stay in sync). What was missing is a guided first-run path —
 * someone who just confirmed their email lands on a blank rule list with
 * no explanation. This wraps the SAME `automation_rules` insert the
 * existing "New Rule" modal does, as 5 short steps instead of one long
 * form, and folds in CV/certification upload (previously only reachable
 * from a separate Vault screen) so setup is genuinely one flow, once.
 *
 * INTEGRATION — add to app/(tabs)/configure.tsx:
 *   import { SetupWizard } from '../../components/onboarding/SetupWizard';
 *   ...
 *   {rules.length === 0 && !isLoading && (
 *     <SetupWizard onComplete={() => queryClient.invalidateQueries({ queryKey: ['automation_rules'] })} />
 *   )}
 *   Place it as an early return / overlay BEFORE the existing tabbed
 *   engine/rules UI renders, so returning users (rules.length > 0) skip
 *   straight past it to the screen that already exists — this component
...
import {
    Sparkles, MapPin, Briefcase, DollarSign, FileText, CheckCircle2,
    ArrowRight, ArrowLeft, Upload, X, Loader2, Award, Wand2,
...
// ── Shared option sets — kept identical to app/(tabs)/configure.tsx ────────
...
export function SetupWizard({ onComplete }: SetupWizardProps) {
    ...
    const starterTemplates: Record<'formal' | 'direct' | 'enthusiastic', string> = useMemo(() => {
        ...
        return {
            formal: `Dear Hiring Team,\n\nI am writing to express my interest in the ${roleLabel} position at [COMPANY]. With a strong background in ${keywords.join(', ') || 'the relevant technologies'}, I am confident I can contribute meaningfully to your team.\n\nI would welcome the opportunity to discuss how my experience aligns with your needs.\n\nSincerely,\n[NAME]`,
            direct: `Hi [COMPANY] team,\n\nI'm applying for the ${roleLabel} role. I work with ${keywords.join(', ') || 'the stack you\'re hiring for'} and I'm looking for my next opportunity to build things that matter.\n\nHappy to walk through my background whenever's useful.\n\n[NAME]`,
            enthusiastic: `Hello!\n\nI just saw the ${roleLabel} opening at [COMPANY] and had to reach out — this looks like exactly the kind of role I've been hoping to find. I bring hands-on experience in ${keywords.join(', ') || 'the technologies this role needs'} and I'd love the chance to bring that energy to your team.\n\nLooking forward to hearing from you!\n\n[NAME]`,
        };
    }, [role, keywords]);
...
const handleActivate = useCallback(async () => {
    ...
    try {
        ...
        const finalLocation = [...locations, customLocation.trim()].filter(Boolean).join(', ') || 'Remote';
...
}, [keywords, role, locations, customLocation, workTypes, experienceLevels, remoteMode, salaryMin, coverLetterDraft, starterTemplates, onComplete]);
                                ...
                                <View className="flex-row flex-wrap gap-2">
                                    {LOCATION_PRESETS.map((loc) => (
                                        <Chip
                                            ...
                                            onPress={() => setLocations((prev) => toggleInArray(prev, loc))}
                                        />
                                    ))}
                                </View>
                                <TextInput
                                    ...
                                    placeholder="Add another city or country..."
                                    placeholderTextColor={C.dim}
                                    style={{
                                        marginTop: 10, color: C.text, fontSize: 14, borderWidth: 1, borderColor: C.border,
                                        borderRadius: 14, paddingHorizontal: 14, paddingVertical: 11, backgroundColor: 'rgba(255,255,255,0.02)',
                                    }}
                                />
                            </View>
                    ...
                    {step === 4 && (
                        <View style={{ gap: 16 }}>
                            ...
                            <View style={{ borderTopWidth: 1, borderTopColor: C.border, paddingTop: 14, marginTop: 4 }}>
                                ...
                                <SummaryLine label="Locations" value={[...locations, customLocation].filter(Boolean).join(', ') || '—'} />
