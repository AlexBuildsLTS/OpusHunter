  ...
  return (
    <Animated.View
      ...
      style={[
        styles.banner,
        {
          borderColor: ok ? `${C.cyan}30` : `${C.pink}30`,
          backgroundColor: ok ? `${C.cyan}08` : `${C.pink}08`,
        },
      ]}
    >
      {ok ? (
      ...
      <Text style={[styles.bannerText, { color: ok ? C.cyan : C.pink }]}>
        {message}
      </Text>
    </Animated.View>
  );
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.certMeta}>
          <View style={styles.certTypeBadge}>
            <Text style={styles.certTypeText}>
              {fileTypeLabel(cert.file_type)}
            </Text>
          </View>
          {cert.file_size_kb && (
            <Text style={styles.certMetaText}>
              {formatFileSize(cert.file_size_kb)}
            </Text>
          )}
          <Text style={styles.certMetaText}>
...
export default function DocumentsScreen() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("certifications")
        .select(
          "id, file_name, file_type, file_size_kb, uploaded_at, storage_path",
        )
        .eq("user_id", user.id)
  ...
  const handleDeleteCert = useCallback(
    async (storagePath: string, id: string) => {
      ...
      >
            ...
            <View style={styles.cvCard}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <View style={styles.cvMeta}>
                  ...
                  >
                    <Text style={[styles.certTypeText, { color: C.cyan }]}>
                      PDF
                    </Text>
                  </View>
                  <CheckCircle2 size={12} color={C.green} />
                  <Text style={[styles.certMetaText, { color: C.green }]}>
                    Active in pipeline
                  </Text>
                </View>
              </View>
          ...
          >
            <View>
              ...
              <Text style={styles.sectionSub}>
                Portfolios, certificates, references
              </Text>
            </View>
            <TouchableOpacity
          ...
          {certsLoading ? (
            ...
            <View style={styles.emptyCerts}>
              ...
              <Text style={styles.emptyCertsSub}>
                Upload portfolios, cover letters, reference letters, or
                certificates.
              </Text>
            </View>
          ) : (
        ...
        <Animated.View entering={FadeIn.delay(400)} style={styles.vaultFooter}>
          ...
          <Text style={styles.vaultFooterText}>
            All files encrypted in Supabase Storage with RLS — only you can
            access them.
          </Text>
        </Animated.View>
      </ScrollView>
