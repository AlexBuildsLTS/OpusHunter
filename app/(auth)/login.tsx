...
function mapAuthError(msg: string): AuthMessage {
  ...
  if (m.includes("already registered") || m.includes("already exists"))
    return {
      type: "warning",
      text: "Email already registered. Sign in instead.",
    };
  ...
  if (m.includes("rate limit"))
    return {
      type: "warning",
      text: "Too many attempts. Please wait a moment.",
    };
    ...
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      ...
      <div
        style={{
          ...
          inset: 0,
          background:
            "radial-gradient(ellipse 65% 55% at 72% 48%, rgba(123,94,167,0.11) 0%, transparent 65%)",
        }}
      />
      {/* @ts-ignore web-only */}
      <div
        style={{
          ...
          inset: 0,
          background:
            "radial-gradient(ellipse 45% 50% at 22% 18%, rgba(0,212,255,0.07) 0%, transparent 60%)",
        }}
      />
      {/* @ts-ignore web-only */}
      <div
        style={{
          ...
          inset: 0,
          background:
            "radial-gradient(ellipse 35% 30% at 80% 85%, rgba(232,67,106,0.05) 0%, transparent 55%)",
        }}
      />
    </View>
  );
...
const FeatureCards = memo(() => (
  <View style={s.featurePanelInner}>
    <Animated.View
      ...
      style={s.featurePanelHeadline}
    >
      <Text style={s.featurePanelTitle}>Why OpusHunter?</Text>
      <Text style={s.featurePanelSub}>
        The intelligent pipeline that works while you sleep.
      </Text>
    </Animated.View>
...
const TERMS_SECTIONS = [
  {
    ...
    items: [
      [
        "Acceptance",
        "By accessing OpusHunter, you agree to these Terms. The platform provides automated job scraping and application execution for enterprise and personal use.",
      ],
      [
        "Acceptable Use",
        "You must not execute denial-of-service attacks on job boards, reverse-engineer the edge architecture, or submit maliciously crafted applications.",
      ],
      [
        "Liability",
        "OpusHunter does not guarantee employment, interview conversions, or ATS bypass success. Automated pipeline use is at your own risk.",
      ],
    ],
  },
  {
    ...
    items: [
      [
        "Data & CV Storage",
        "Your encrypted CVs are stored in a secure Supabase Vault. Documents are parsed only for job matching and never sold to third parties.",
      ],
      [
        "Credential Security",
        "All credentials are encrypted at rest. You retain full data sovereignty and may permanently delete your Vault at any time.",
      ],
    ],
  },
];

const TermsModal = memo(
  ({
    visible,
    onClose,
    onAccept,
          ...
          >
            ...
            <View style={s.termsFooter}>
              ...
              <TouchableOpacity
                ...
                style={[
                  s.termsAcceptBtn,
                  canAccept ? s.termsAcceptActive : s.termsAcceptDisabled,
                ]}
                activeOpacity={0.85}
              >
                <Text
                  style={[
                    s.termsAcceptText,
                    { color: canAccept ? "#020507" : "rgba(255,255,255,0.2)" },
                  ]}
                >
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (mode === "sign-in") {
      ...
      if (!v.valid) {
        ...
        return setMessage({
          type: "error",
          text: v.error || "Validation failed",
        });
      }
      ...
      {success !== "none" ? (
        ...
        <>
            ...
            {(["sign-in", "sign-up"] as AuthMode[]).map((m) => (
              ...
              >
                <Text
                  style={[
                    s.tabText,
                    mode === m ? s.tabTextActive : s.tabTextInactive,
                  ]}
                >
          ...
          {mode === "sign-up" && (
            ...
            >
              <View style={{ marginTop: 12 }}>
                ...
                <TouchableOpacity
                  onPress={() =>
                    agreedToTerms ? setAgreedToTerms(false) : setShowTerms(true)
                  }
                  activeOpacity={0.7}
                  style={s.termsRow}
                >
          ...
          {message && success === "none" && (
            ...
            >
              {message.type === "error" && (
                <AlertCircle size={15} color={T.pink} />
              )}
              {message.type === "warning" && (
                <AlertTriangle size={15} color={T.amber} />
              )}
              {message.type === "success" && (
                <CheckCircle2 size={15} color={T.cyan} />
              )}
              <Text
                style={[
                  s.bannerText,
